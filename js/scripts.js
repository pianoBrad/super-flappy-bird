...
var bounce_height = 100;
var bounce_duration = 350;
var pipe_width = 85;
var pipe_top_height = 35;
var pipe_bottom_height = 35;
...
var is_pipe_still_next = function() {
	var current_pipe_position = get_current_position( $('.pipe-wrap.next') );
	...
	if ( (current_pipe_position.left + pipe_width) < current_bird_position.left ) {
		var cleared_pipe = $('.pipe-wrap.next');
		var next_pipe = cleared_pipe.next('.pipe-wrap')
		cleared_pipe.removeClass('next');
		next_pipe.addClass('next');
	}
}

var is_bird_touching = function() {
	var pipe_top_x = $('.pipe-wrap .pipe.top').outerHeight();
	var pipe_bottom_x = world_height - $('.pipe-wrap .pipe.bottom').outerHeight();
	...
	if ( current_bird_position_top > pipe_top_x && current_bird_position_bottom < pipe_bottom_x ) {
		console.log('fuck');
		return false;
	} else {
